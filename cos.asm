...
# argumentos:  x = $f14 , n = $f16 , cos(x) = $f30
seno:	
	move $s0, $ra                           #salva $ra 
	ldc1 $f8, um
...
loop_seno:
	...
	######################################### Calcula : fatorial( 2 *  n  )
	mov.d $f0, $f22				# chama fat( 2 *  n  )
	jal fatorial				#
	mov.d $f24, $f4
	...
	######################################### Calcula : (((-1)^n ) / ( 2 * n  )!) * x^( 2 *  n  )
	mul.d $f28, $f28, $f26 			#

	######################################### Calcula : somatorio de (((-1)^n ) / ( 2 * n  )!) * x^( 2 *  n ), 0..n
	add.d $f30, $f30, $f28 			#
...
return_fatorial_zero:
	ldc1 $f4, um				# retorna 1
	move $ra, $s2                           #restaura $ra
	jr $ra
